...
def get_args_parser():
    parser = argparse.ArgumentParser('Training script for image classification', add_help=False)
...
def main(args):
    ...
    print('=============================== after moving EMA', torch.cuda.memory_allocated())
    if args.distributed:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.output_dir and args.save_ckpt:
            ...
            utils.save_model(
                args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                loss_scaler=loss_scaler, epoch=epoch, model_ema=model_ema, is_latest=True)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('Training script for image classification', parents=[get_args_parser()])
